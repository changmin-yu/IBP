...
class IndianBuffetProcessInfiniteVI:
    ...
    def initialise(self):
        ...
        self.rho = np.ones((self.N, self.K)) * 0.5
    ...
    def free_energy(self):
        ...
        for k in range(self.K):
            exp_log_p_Z += np.sum(
                self.rho[:, k] * np.sum(digamma(self.a[:k]) - digamma(self.a[:k] + self.b[:k])) + 
        ...
        neg_exp_log_q_A = np.sum(self.D / 2 * np.log(2 * np.pi * np.exp(1)) + self.D / 2 * self.sigmasq_A_variational)
        neg_exp_log_q_Z = np.sum(
            -self.rho * np.log(self.rho) - (1 - self.rho) * np.log(1 - self.rho)
        )
    ...
    def update_Z(self):
        for n in range(self.N):
            for k in range(self.K):
                logit = (
                    -1 / (2 * self.sigmasq_n) * np.sum(self.sigmasq_A_variational + np.square(self.m[k, :])) - 
    ...
    def update_v(self):
        for k in range(self.K):
            ...
            for m in range(k, self.K):
                multinomial_probs = self.variational_multinomial(m)
                if m > k:
    ...
    def update_A(self):
        for k in range(self.K):
            ...
            self.m[k] = self.sigmasq_A_variational/  self.sigmasq_n * np.sum(self.rho[:, [k]] * (self.X - (np.matmul(self.rho, self.m) - np.outer(self.rho[:, k], self.m[k]))), axis=0)
            
    def exp_log_p_X_cond_Z_A(self):
        constant = -self.D / 2 * np.log(2 * np.pi * self.sigmasq_n)
