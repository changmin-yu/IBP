...
class IndianBuffetProcessFiniteVI:
    def __init__(
        self, 
        K: int, # truncation of stick-breaking
        X: np.ndarray, 
    ...
    def free_energy(self):
        log_p_X_cond_Z_A = self.exp_log_p_X_cond_Z_A()
        exp_log_p_pi = np.sum(
            np.log(self.alpha / self.K) + (self.alpha / self.K - 1) * (digamma(self.a) - digamma(self.a + self.b))
        )
        exp_log_p_A = -self.D * self.K / 2 * np.log(2 * np.pi * self.sigmasq_A) 
        for k in range(self.K):
            exp_log_p_A -= 1 / (2 * self.sigmasq_A) * np.sum(np.square(self.m[k]) + self.sigmasq_A_variational[k])
        ...
        for k in range(self.K):
            exp_log_p_Z += np.sum(
                self.rho[:, k] * (digamma(self.a[k]) - digamma(self.a[k] + self.b[k])) + 
                (1 - self.rho[:, k]) * (digamma(self.b[k]) - digamma(self.a[k] + self.b[k]))
            )
        
        neg_exp_log_q_pi = np.sum(
            gammaln(self.a) + gammaln(self.b) - gammaln(self.a + self.b) - 
            (self.a - 1) * digamma(self.a) - 
            (self.b - 1) * digamma(self.b) + 
            (self.a + self.b - 2) * digamma(self.a + self.b)
        ...
        return log_p_X_cond_Z_A + exp_log_p_pi + exp_log_p_A + exp_log_p_Z + neg_exp_log_q_pi + neg_exp_log_q_A + neg_exp_log_q_Z
    ...
    def update_pi(self):
        for k in range(self.K):
            self.a[k] = self.alpha/  self.K + np.sum(self.rho[:, k])
            self.b[k] = 1 + self.N - np.sum(self.rho[:, k])
